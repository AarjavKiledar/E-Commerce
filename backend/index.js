...
const storage = multer.diskStorage({
    destination: path.join(__dirname, 'public/upload/images'), // Store in public/upload/images
    filename: (req, file, cb) => {
        return cb(null, `<span class="math-inline">\{<4\>file\.fieldname\}\_</span>{Date.now()}${path.extname(file.originalname)}`);
    }
});
...
app.use('/images', express.static(path.join(__dirname, 'public/upload/images'))); // Serve from public

app.post("/upload", upload.single('product'), (req, res) => {
    res.json({
        success: 1,
        image_url: `/images/${req.file.filename}` // Relative URL
    });
});
...
const fetchUser = async (req, res, next) => {
    ...
    if (!token) {
        return res.status(401).send({ errors: "Please authenticate using valid token" }); // Important: Return to stop further execution
    }
